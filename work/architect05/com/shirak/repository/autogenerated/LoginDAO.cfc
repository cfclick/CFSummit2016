component output="false"
{
	...
	/* create */
	public any function create( string UserName, string Password, string Createdby= '', date CreatedDateTime= '01/01/1900', string UpdatedBy= '', date UpdatedDateTime= '01/01/1900' )
	{
		...
		q.setsql('INSERT INTO Auth.Login
			(
				username,password,createdby,createddatetime,
		...
		if(arguments.LoginID NEQ '')
			IdentityCol=arguments.LoginID;
	...
	/* read */
	public Login function read(id)
	    {
		
		/* Auto-generated method
		         Add authroization or any logical checks for secure access to your data */
		var obj = createObject('component', 'Login').init();
		var i = 1;
		var qry="";
		
		var q= new query();
		q.setdatasource("dsnConference");
		q.setsql('SELECT loginid,username,password,createdby,createddatetime,
					updatedby,updateddatetime
			FROM Auth.Login
			where loginid = "#ARGUMENTS.id#"');
		qry=q.execute().getresult();	
		/* load value object */
		obj.setLoginID(qry.LoginID[i]);
		obj.setUserName(qry.UserName[i]);
		obj.setPassword(qry.Password[i]);
	...
	/* update */
	public void function update(numeric LoginID,	string UserName,	string Password,	string Createdby,	date CreatedDateTime,	string UpdatedBy,	date UpdatedDateTime)
		{
		/* Auto-generated method
		         Add authroization or any logical checks for secure access to your data */
		...
		q.setsql('UPDATE Auth.Login
			...
			WHERE loginid = "#ARGUMENTS.LoginID#" ');
	    qry=q.execute().getresult();		
	...
	/* delete */
	public void function delete(numeric LoginID)
		{
		/* Auto-generated method
		         Add authroization or any logical checks for secure access to your data */
		var qry="";
		...
		var q =new query();
		q.setdatasource("dsnConference");
		q.setsql('DELETE FROM Auth.Login
			WHERE loginid = "#ARGUMENTS.LoginID#"' );
		qry=q.execute().getresult();
