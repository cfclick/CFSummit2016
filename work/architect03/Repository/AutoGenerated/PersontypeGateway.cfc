component output="false"
{
	...
	{
		var INSTANCE = {};
		INSTANCE.dao = createObject('component', 'PersontypeDAO').init();
		return this;
	}

	/* getAll */
	public Persontype[] function getAll()
	{
		var collection = [];
		var obj = '';
		var qry = '';
		...
		q.setsql('SELECT * FROM Definition.PersonType');
		qry=q.execute().getresult();
		/* load value objects */
		for(i=1;i<="#qry.recordcount#";i++)
		{
			obj = createObject('component', 'Persontype').init();
	...
	/* getAll_paged */
	public Persontype[] function ngetAll_paged(numeric start,numeric count)
	{
		var collection = [];
		var obj = '';
		var qry = '';
	...
	public numeric function count()
	{
		...
		var q=new query();
		q.setdatasource(dsnConference);
		q.setsql('SELECT COUNT(PersonTypeID) AS total
