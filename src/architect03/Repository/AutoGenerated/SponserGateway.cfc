component output="false"
{
	...
	{
		var INSTANCE = {};
		INSTANCE.dao = createObject('component', 'SponserDAO').init();
		return this;
	}

	/* getAll */
	public Sponser[] function getAll()
	{
		var collection = [];
		var obj = '';
		var qry = '';
		...
		q.setsql('SELECT * FROM Conf.Sponser');
		qry=q.execute().getresult();
		/* load value objects */
		for(i=1;i<="#qry.recordcount#";i++)
		{
			obj = createObject('component', 'Sponser').init();
			if( !isNull( qry.Sponserid[i] ) && len( qry.Sponserid[i] ) ) obj.setSponserid(qry.Sponserid[i]);
	...
	/* getAll_paged */
	public Sponser[] function ngetAll_paged(numeric start,numeric count)
	{
		var collection = [];
		var obj = '';
		var qry = '';
	...
	public numeric function count()
	{
		...
		var q=new query();
		q.setdatasource(dsnConference);
		q.setsql('SELECT COUNT(SponserID) AS total
			FROM Conf.Sponser');
		qry=q.execute().getresult();
		return qry.total[1];
	}
